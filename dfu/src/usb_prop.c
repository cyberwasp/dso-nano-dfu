...
RESULT DFU_Get_Interface_Setting(u8 Interface, u8 AlternateSetting)
{
  ...
  {
    return USB_UNSUPPORT;   /* In this application we don't have more tha 2 AlternateSetting */
  }
  else if (Interface > 0)
  {
...
u8 *UPLOAD(u16 Length)
{
  ...
  else if (wBlockNum > 1)
  {
    ...
    if (Addr < 0x800000)
    { 
      /* The SPI flash is not a memory zone -> read request is needed */
      SPI_FLASH_BufferRead(&Load_Buffer[0], Addr, wlength);
      Phy_Addr = &Load_Buffer[0];
    }
    else if (Addr > 0x800000) /* internal flash */
    {
      Phy_Addr = (u8*)(Addr);
    }
...
//          DeviceStatus[1] = (wlength >> 8) * 2  ; /* Nb of Pages(of 256 )* 2ms */
          DeviceStatus[1] = (wlength >> 8) * 20  ; /* Nb of Pages(of 256 )* 20ms */
          DeviceStatus[2] = 0;
          DeviceStatus[3] = 0;
        }
    ...
    case   STATE_dfuMANIFEST_SYNC :
      ...
      else if (Manifest_State == Manifest_complete && DFU_ConfigDescriptor[20]
               & 0x04)
      {
        DeviceState = STATE_dfuIDLE;
        DeviceStatus[4] = DeviceState;
